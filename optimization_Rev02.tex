...
%Job scheduling formulations are designed to find a solution following a set of constraints while trying to minimize or maximize an objective function. The objective function defines the behavior of the scheduling algorithm, so it must be specifically tailored for the system we are scheduling. By that we mean that the objective function must incentivate strategies that benefits the system and avoid strategies that are undesired on the system. Practical systems are often difficult to evaluate as they present complex and dynamic behavior. When validating software, for example, some critical issues must be validated and released immediately, while routine updates must be scheduled in specific dates, and other features have the necessity of being scheduled just before a specific due date. However, most formulations are modeled for specific systems, and present trouble when dealing with mixed strategies, often needing a reformulation adding new terms and constraints to fit the problem. In this paper we present a parametric formulation that enables the scheduling behavior to be dictated by the items being scheduled, instead of only the objective function. We'll demonstrate the proposed model's ability to shift its behavior according to different strategies without reformulation, providing an adaptative model capable of handling scheduling on dynamic environments.
...
%Job scheduling formulations optimize task allocation by respecting constraints while minimizing or maximizing an objective function. The objective function dictates scheduling priorities, incentivizing desirable behaviors and discouraging inefficiencies. However, real-world systems, such as software validation, require scheduling flexibility to accommodate diverse and evolving priorities. Critical issues must be validated and released immediately, routine updates follow predefined schedules, and feature releases require just-in-time validation. Traditional scheduling models are often rigid, requiring reformulation with additional constraints to handle mixed strategies effectively. This paper proposes a parametric scheduling formulation where scheduling behavior is determined by the properties of individual tasks rather than being explicitly encoded in the objective function. By adjusting parameter matrices, the proposed Integer Linear Programming (ILP) model seamlessly adapts to different scheduling paradigms without structural modifications. Experimental results demonstrate its ability to match global optima across various scheduling strategies while effectively handling mixed-priority tasks. This approach provides a flexible and adaptive solution for dynamic environments where scheduling priorities continuously evolve.
